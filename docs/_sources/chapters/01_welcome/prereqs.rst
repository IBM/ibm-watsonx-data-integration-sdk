...
Setup and installation for the ``ibm-watsonx-data-integration`` SDK for Python is lightweight and straightforward, with only a few dependencies required by the package itself.

For a smooth installation and optimal user experience, please review the prerequisites below before installing the SDK and make sure all requirements have been met.
...
* You must have an account with access to the watsonx.data integration platform and an ``IAM API key`` created on that account to :ref:`authenticate <getting_started_and_tutorials__authentication>` from the SDK.
...
* (Optional but strongly encouraged) For the vast majority of operations on the watsonx.data integration platform, such as executing a flow, you will need to generate a ``User API key`` within the watsonx.data integration service.
