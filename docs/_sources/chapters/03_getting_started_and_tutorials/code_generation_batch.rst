...
:py:class:`ibm_watsonx_data_integration.services.datastage.codegen.python_generator.PythonGenerator` is an entry point for converting existing flows to Python SDK code.
...
- ``persist_topology``: If set to ``True``, this maintains the original topology of the flow, including comments and annotations. Otherwise, the flow's stages are auto-arranged. Defaults to ``False``.
...
Both of these formats can be obtained in several ways, but one option is to download them directly from the flow editor.
...
For complete documentation on compilation modes, materialization policies, and usage examples, see :ref:`preparing_data__batch__compilation_mode`.
