...
An access group can be created by passing ``name``, ``description``, and ``roles`` to the :py:meth:`Platform.create_access_group_on_prem() <ibm_watsonx_data_integration.platform.Platform.create_access_group_on_prem>` method.
This method returns an :py:class:`~ibm_watsonx_data_integration.cpd_models.access_groups_model_on_prem.AccessGroupOnPrem` object.
...
Access groups can be retrieved using the :py:attr:`Platform.access_groups <ibm_watsonx_data_integration.platform.Platform.access_groups>` property.
...
To update an access group, first modify the ``name`` and ``description`` fields of an existing :py:class:`~ibm_watsonx_data_integration.cpd_models.access_groups_model_on_prem.AccessGroupOnPrem` object.
Then pass the updated :py:class:`~ibm_watsonx_data_integration.cpd_models.access_groups_model_on_prem.AccessGroupOnPrem` object to the :py:meth:`Platform.update_access_group() <ibm_watsonx_data_integration.platform.Platform.update_access_group>` method.
