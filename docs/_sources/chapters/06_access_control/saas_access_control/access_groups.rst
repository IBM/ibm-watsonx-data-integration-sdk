...
To get a specific access group using the SDK, provide the ``name`` as a filter to the :py:attr:`Platform.access_groups <ibm_watsonx_data_integration.platform.Platform.access_groups>` by using the :py:meth:`AccessGroups.get() <ibm_watsonx_data_integration.common.models.CollectionModel.get>` function.
...
:py:meth:`AccessGroup.add_members_to_access_group() <ibm_watsonx_data_integration.cpd_models.access_groups_model.AccessGroup.add_members_to_access_group>` function. This will add the specified member(s) to the desired Access Group and returns an API response, whose status code should be ``<200>`` if the member(s) were successfully added.
...
In the SDK, all current members of the selected access group can be retrieved by calling the :py:meth:`AccessGroup.get_access_group_members() <ibm_watsonx_data_integration.cpd_models.access_groups_model.AccessGroup.get_access_group_members>` function.
...
pass the member to the :py:meth:`AccessGroup.check_membership() <ibm_watsonx_data_integration.cpd_models.access_groups_model.AccessGroup.check_membership>` function. This will return an API response, whose status should be ``<204>`` if the member does possess membership to the specified access group and ``<404>`` if it does not possess membership.
